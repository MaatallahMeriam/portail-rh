...
import com.example.PORTAIL_RH.equipe_service.Service.EquipeService;
...
public class EquipeServiceImpl implements EquipeService {
    ...
    public void deleteEquipe(Long id) {
        ...
        // Create a copy of users to avoid ConcurrentModificationException
        List<Users> usersCopy = new ArrayList<>(equipe.getUsers());
        usersCopy.forEach(user -> {
            user.setEquipe(null);
            equipe.getUsers().remove(user);
            if (equipe.getManager() != null && user.getId().equals(equipe.getManager().getId())) {
                user.getEquipesGerees().remove(equipe);
                equipe.setManager(null);
            }
            usersRepository.save(user); // Single save per user
        });
